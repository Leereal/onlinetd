...
class ManageDepositController extends Controller
{
    ...
    public function pdeposit($id)
    {
        ...
        $account_bal = $user->account_bal;
        $depositBonus = $settings->deposit_bonus;   
        $earnings = floatval($referral_commission > 0 ? ($referral_commission/100 * $amount) : 0);
        $bonus = intval($depositBonus > 0 ? ($depositBonus/100 * $amount) : 0);
        $funds = $amount + ($account_bal + $depositBonus > 0 ? ($depositBonus/100 * $amount) : 0);
